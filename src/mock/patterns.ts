...
export interface PatternMockType {
  name: PatternKey
  label: string
  pattern: PatternType
  minimizedPattern: PatternType
  rows: number
  cols: number
  cellSize: number
}

const createMinimizedPattern = (rows: number, cols: number, pattern: PatternType): PatternType => {
  const middleRow = Math.floor(rows / 2)
  const middleCol = Math.floor(cols / 2)

  const output: PatternType = []
